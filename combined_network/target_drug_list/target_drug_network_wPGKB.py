...
# create a duplicated node list if drug is in both DB_culled_drug_set AND PGKB_drugID_set
def check_duplicates(DB_culled_drug_set):
    duplicates = set()
    for drug in DB_culled_drug_set:
        if any(PGKB_drugID in drug for PGKB_drugID in PGKB_drugID_set):
            duplicates.add(drug)
    return duplicates

duplicate_nodes = check_duplicates(DB_drug_set)
...
g = nx.Graph()
g.add_nodes_from(DB_culled_drug_set,viz={'color':{'r':200,'g':10,'b':10,'a':0.5},'size':15})
g.add_nodes_from(duplicate_nodes,viz={'color':{'r':100,'g':50,'b':10,'a':0.5},'size':25})
g.add_nodes_from(culled_gene_set,viz={'color':{'r':10,'g':10,'b':200,'a':0.5},'size':15})
g.add_nodes_from(MM_culled_disease_set,viz={'color':{'r':10,'g':200,'b':10,'a':0.5},'size':15})
g.add_edges_from(DB_culled_drug_data_set)
g.add_edges_from(MM_culled_disease_data_set)

# make sure to updated fileNAME
nx.write_gexf(g,'anticoagulant_network_v2.gexf',version='1.2draft')
